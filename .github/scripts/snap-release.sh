...
#
# Music Assistant versions are bare (2.9.8 — no `v` prefix), unlike zwave-js-ui's
# v-prefixed ones; the parser accepts both. Tracks keep the `v<major>.<minor>`
# naming (v2.9) to match the store track-creation guardrail pattern.
#
# Subcommands:
#   version-to-track <ver>             2.9.8|v2.9.8 -> v2.9          (""->"")
#   major <ver>                        2.9.8 -> 2                    (""->"")
#   channel-version <channel>          (stdin: `snapcraft status`) -> version or "" (↑/--/- -> "")
#   branch-has-revisions <track> <pr>  (stdin: `snapcraft status`) -> yes|no
#   needs-stable-bump <new> <cand>     yes iff cand set & line(new) > line(cand), where
#                                      line = v<major>.<minor>. (Upstream lives on major 2
#                                      indefinitely, so a major-only rule would freeze
#                                      latest/stable; a new MINOR line bumps it instead.)
...
case "$cmd" in
  ...
  major)
    ...
    ;;
  channel-version)
    # stdin = `snapcraft status`; arg = channel like latest/candidate or v2.9/edge/7.
    ...
    # read the Channel as the field immediately before it. The version match requires
    # digits-then-dot (`2.9.8`, optionally v-prefixed) so bare Revision integers and
    # Expires-at timestamps never match.
    ch="${1:-}"
    awk -v t="${ch%%/*}" -v c="${ch#*/}" '
  ...
  needs-stable-bump)
    ...
    nt="$(bash "$0" version-to-track "$new")"
    ct="$(bash "$0" version-to-track "$cand")"
    if [ "$(_vercmp "$nt" "$ct")" -gt 0 ]; then echo yes; else echo no; fi
    ;;
  is-at-least)
