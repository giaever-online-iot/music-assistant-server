...
run() {
  ...
  out="$(SNAPCRAFT_SESSION_COOKIE="$cookie" MOCK_BODY="$body" MOCK_CODE="$code" MOCK_REDIRECT="$redirect" \
        bash "$SCRIPT" music-assistant-server v2.9 2>&1)"
  rc=$?
...
run "302 -> login = expired"     "c"   ""                          "302" "https://snapcraft.io/login?next=/music-assistant-server/create-track" 1 "invalid or expired"
