...
case ",\$archs," in *,arm64,*) printf 'OK' > music-assistant-server_2.9.8_arm64.snap ;; esac
case ",\$archs," in *,armhf,*) printf 'OK' > music-assistant-server_2.9.8_armhf.snap ;; esac
case ",\$archs," in *,amd64,*)
  case "$mode" in
    flaky)   if [ "\$n" -le 1 ]; then printf 'CORRUPT' > music-assistant-server_2.9.8_amd64.snap; exit 1; else printf 'OK' > music-assistant-server_2.9.8_amd64.snap; fi ;;
    always)  printf 'CORRUPT' > music-assistant-server_2.9.8_amd64.snap; exit 1 ;;
    partial) exit 1 ;;
  esac ;;
esac
EOF
