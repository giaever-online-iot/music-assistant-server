...
export SNAP_NAME=music-assistant-server
fail=0
...
has   "all: install command"     "$all" "sudo snap install music-assistant-server --channel=$CH"
...
has "default snap name when SNAP_NAME unset" "$dflt" "sudo snap install music-assistant-server --channel=$CH"
